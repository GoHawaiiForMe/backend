...
export default class ChatRoomService {
  ...
  async getChatRoomById(userId: string, chatRoomId: string): Promise<ChatRoomProperties> {
    ...
    return chatRoom.toClientSingle();
  }

  async postChatRoom(userId: string, planId: string): Promise<ChatRoomProperties> {
    const exists = await this.chatRoomRepository.exists(planId);
    if (exists) {
      throw new ConflictError(ErrorMessage.CHAT_ROOM_POST_CONFLICT);
    }

    const userIds = await this.planService.checkPlanAndGetChatParticipants(planId);

    if (!userIds.includes(userId)) {
      throw new ForbiddenError(ErrorMessage.CHAT_ROOM_FORBIDDEN_ID);
    }

    const chatRoomData = ChatRoom.create({ userIds, planId });
